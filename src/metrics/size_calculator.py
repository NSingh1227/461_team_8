...
class SizeCalculator(MetricCalculator):
    ...
    def calculate_score(self, context: ModelContext) -> float:
        ...
        if context.model_url and context.model_url.startswith("https://huggingface.co"):
            try:
                self.platform_compatibility = self._get_intelligent_fallback_scores(
                    context)
                score = max(self.platform_compatibility.values()
                            ) if self.platform_compatibility else 0.5
                end_time = time.time()
        ...
        try:
            total_artifact_size_mb: Optional[float] = self._estimate_artifact_size_mb(
                context)
            ...
            if total_artifact_size_mb is None:
                try:
                    self.platform_compatibility = self._get_intelligent_fallback_scores(
                        context)
                    score = max(
                        self.platform_compatibility.values()) if self.platform_compatibility else 0.5
                except Exception:
    ...
    def _get_intelligent_fallback_scores(
            self, context: ModelContext) -> Dict[str, float]:
            ...
            if isinstance(siblings, list):
                for s in siblings:
                    filename: str = str(
                        getattr(
                            s, "rfilename", "") or (
                            s.get("rfilename") if isinstance(
                                s, dict) else ""))
                    size_bytes: Optional[int] = getattr(
                        s, "size", None) or (
                        s.get("size") if isinstance(
                            s, dict) else None)

                    if filename and self._looks_like_artifact(
                            str(filename)) and size_bytes:
    ...
    def _download_and_analyze_config(
            self, repo_id: str, filename: str) -> Dict[str, Any]:
        try:
    ...
    def _estimate_model_parameters(self, config: Dict[str, Any]) -> int:
        try:
            if not isinstance(config, dict):
                print(
                    f"SizeCalculator: config is not a dictionary: {type(config)}", file=sys.stderr)
                return 0
            ...
            ffn_params: int = num_layers * \
                (2 * hidden_size * intermediate_size)  # Feed-forward network
